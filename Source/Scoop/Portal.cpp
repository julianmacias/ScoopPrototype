...
void APortal::Setup()
{
	...
	CHECK_DESTROY(LogPortal, !PC, "Player portal character could not be found in the portal class %s.", *GetName());
	portalChar = character;
...
void APortal::UpdatePortalView()
{
	...
	portalCapture->PostProcessSettings = portalChar->camera->PostProcessSettings;
	...
	UCameraComponent* playerCamera = portalChar->camera;
...
void APortal::UpdateTrackedActors()
{
	...
	if (actorsBeingTracked > 0)
	{
		...
		for (TMap<AActor*, FTrackedActor>::TIterator trackedActor = trackedActors.CreateIterator(); trackedActor; ++trackedActor)
		{
			...
			if (APortalCharacter* isPlayer = Cast<APortalCharacter>(trackedActor->Key)) continue;
...
void APortal::CopyActor(AActor* actorToCopy)
{
	...
	if (APortalCharacter* isChar = Cast<APortalCharacter>(newActor))
	{
		isChar->GetCapsuleComponent()->SetCollisionResponseToChannel(ECC_PortalBox, ECR_Ignore);
		isChar->GetCapsuleComponent()->SetCollisionResponseToChannel(ECC_Pawn, ECR_Ignore);
		//isChar->GetCapsuleComponent()->SetSimulatePhysics(false);
		isChar->PrimaryActorTick.SetTickFunctionEnable(false);
	}
